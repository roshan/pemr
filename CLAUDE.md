...
- **The response is encoded in the POST URL** (`…/milestones/mark/{key}/{response}`), NOT a form field — htmx serialises forms with `new FormData(form)`, which drops submit-button `name=value`, so a shared-form + `<button type=submit name=response>` design silently sends no response (this regressed once). Small HTMX attribute set only (`hx-post`/`hx-get`/`hx-target`/`hx-swap`/`hx-trigger`).
