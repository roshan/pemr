...
**6. `feature_registry` (per-subject OPT-IN modules)** — `src/feature_registry.rs`. One `FeatureDef { key, label, description, surface }` per opt-in module the user can add to a subject's chart; **enablement** is a `subject_features` row (migration 0018), the **catalogue** is this in-code list (a descriptor list, NOT a plugin runtime). The chart's "Add feature…" picker iterates the not-yet-enabled entries; enabling/removing HTMX-swaps the `#subject-features` area so the surface pops in with no reload. Removing a feature deletes its enablement row (hides the surface) but **never** touches the module's data — disable, never delete. Entries: `milestones` (PEMR-35, `Surface::Inline`), `growth` (PEMR-47, `Surface::Nav` — gates the "Growth charts" header button + `/growth` route + mini chart card; migration 0020 enables it for **Astra only**), and `allergies` (`Surface::Inline` — gates the clinical-summary allergy card; migration 0021 enables it for **every subject that has allergy rows**, so recorded allergies never vanish)… + `/subjects/{id}/growth` route + the chart's mini growth card; migration 0020 enables it for **Astra only**, per the 2026-08-24 family decision). Distinct from `subject_modules` (the always-on clinical cards): feature modules are gated + i…nteractive. Growth (PEMR-47) will register as a second entry when it's gated. **Add an opt-in module = one `FeatureDef` + a render arm in `handlers::milestones::render_feature_area`.**
