...
- **UI shape (`handlers`/`views::milestones`):** the chart's feature area shows a **compact summary card** (current checkpoint + completion meter + disclaimer + "Open milestones →"); the **interactive checklist lives on its own page**, `/subjects/{id}/milestones`. Each milestone is mark-able yes / not-yet / no via HTMX (`#milestone-checklist` swap); marking "yes" reveals an observed-on date that saves on change (defaults to today). Prev/next steppers browse adjacent checkpoints. **The response is encoded in the POST URL** (`…/milestones/mark/{key}/{response}`), NOT a form field — htmx serialises forms with `new FormData(form)`, which drops submit-button `name=value`, so a shared-form + `<button type=submit name=response>` design silently sends no response (this regressed once). Small HTMX attribute set only (`hx-post`/`hx-get`/`hx-target`/`hx-swap`/`hx-trigger`).
