...
- **Checklist UI (`handlers`/`views::milestones`):** inline on the chart, gated by the feature. Each milestone is mark-able yes / not-yet / no via HTMX (form POST → upsert → `#milestone-checklist` swap); marking "yes" records an editable observed-on date (defaults to today). Prev/next steppers browse adjacent checkpoints. Small HTMX attribute set only (`hx-post`/`hx-get`/`hx-target`/`hx-swap`).
