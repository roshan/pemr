...
- **One first-party JS exception:** the `/timeline` page carries a small inline `<script>` (`WHEEL_ZOOM_JS` in `views/dashboard.rs`) for **cursor-centred scroll-wheel zoom** — wheel direction + pointer position can't be read in CSS or htmx. It reads the current window + data bounds off `#tl-zoom`, computes the date under the cursor, and navigates to an explicit `from`/`to` window (re-centred so that date stays put), shrinking/growing the span ~40% a tick; at the fully-zoomed-out end it lets the page scroll. It also wires the from/to date boxes. The window is server-driven (`from`/`to` ISO params beat the `range` preset); the JS only navigates. Keep custom JS to this kind of last-resort, no-dependency, no-build snippet; reach for htmx first.
