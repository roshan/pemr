...
- **One first-party JS exception:** the `/timeline` page carries a small inline `<script>` (`WHEEL_ZOOM_JS` in `views/dashboard.rs`) for **cursor-centred scroll-wheel zoom** — wheel direction + pointer position can't be read in CSS or htmx. It only intercepts the wheel when the pointer is over the timeline strip (`[data-tl-band]` rect ± a margin) — otherwise the page scrolls normally; and once fully zoomed out it stops intercepting. When zooming, it reads the current window off `#tl-inner` + data bounds off `#tl-zoom`, computes the date under the cursor, and **`htmx.ajax`-swaps `#tl-inner` in place** (no reload) to a re-centred `from`/`to` window (span ±25%/tick, the cursored date stays put). A **zoom-in that would empty the window is refused**: every marker carries its date in `data-d`, and since a zoom-in window is always a subset of the current one, if no `data-d` falls inside the proposed `[from,to]` the step is dropped — so you stop at the tightest *populated* window instead of falling into a blank gap. The empty-state (a `range` preset with no events) has **no `[data-tl-band]`**, so the handler falls back to `#tl-inner`'s rect there, keeping zoom-out alive. The window is server-driven: `/timeline` returns the full page normally and just the `#tl-inner` partial on an `HX-Request`; `from`/`to` ISO params beat the `range` preset; the tabs and the from/to date-box form also htmx-swap `#tl-inner`. The JS only computes the window and calls `htmx.ajax`. **htmx loads with `defer`, so this inline script defers its own wiring to `DOMContentLoaded`** — otherwise `window.htmx` is undefined when it runs at parse time and the listener never attaches (this regressed once). Keep custom JS to this kind of last-resort, no-dependency, no-build snippet; reach for htmx first.
