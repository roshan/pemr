...
- **Export triage (measured 2026-08-23 on `~/health-exports/ucsf-ehi-2026-07/`):** 3,920 tables ship, only **296 have rows**. The bulk of the remainder is access-audit logs (13 tables / 14,879 rows) and billing/claims (45 / 2,571) — both skippable. `DOC_INFORMATION`'s 42 "encounter documents" are **registration paperwork, not clinical** (28 scanned photo/non-photo IDs, 3 coverage cards, HIPAA notices, ToS). Labs are confirmed narrative-only: `ORDER_RESULTS` is empty and values like bilirubin appear only inside the RTF prose. Full triage in `KB:PEMR:import`.
